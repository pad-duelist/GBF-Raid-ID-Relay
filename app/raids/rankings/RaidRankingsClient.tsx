// app/raids/rankings/RaidRankingsClient.tsx
"use client";
...
export default function RaidRankingsClient() {
  ...
  const mergePosters = (rows: Poster[]): Poster[] => {
    ...
    for (const r of rows) {
      ...
      if (aT && bT) {
        ...
        nextName = r.user_name; // フォールバック：後勝ち
      }
                ...
                {posters.length === 0 ? (
                  ...
                  posters.map((p, i) => (
                    <tr key={`${p.sender_user_id ?? "null"}-${i}`} className="border-t border-white/10">
                      ...
                      <td className="px-3 py-2">{p.user_name?.trim() ? p.user_name : "(no name)"}</td>
